        ...
        <Animated.View style={[styles.card, { backgroundColor: colors.surface, opacity: contentOpacity, transform: [{ translateX: contentTranslateX }] }]}> 
          {step === 1 && (
            <>
              ...
              <TouchableOpacity style={[styles.primaryButton, { backgroundColor: colors.primary }]} onPress={handleEmailContinue}>
          ...
          {step === 2 && (
            <>
              <Text style={[styles.helper, { color: colors.text, opacity: 0.8 }]}>Te enviamos un código de 5 dígitos a {email}</Text>
          ...
          {step === 3 && (
            <>
              ...
              <View style={styles.strengthTrack}><View style={[styles.strengthFill, { width: `${Math.min(100, ( (password.length >= 8 ? 1 : 0) + (/\d/.test(password) ? 1 : 0) + (/[^\w\s]/.test(password) ? 1 : 0)) / 3 * 100)}%`, backgroundColor: (password.length >= 8 && /\d/.test(password) && /[^\w\s]/.test(password)) ? colors.success : (/\d/.test(password) || /[^\w\s]/.test(password)) ? colors.warning : colors.error }]} /></View>
              <View style={{ gap: 6 }}>
              ...
              <TouchableOpacity style={[styles.primaryButton, { backgroundColor: (password.length >= 8 && /\d/.test(password) && /[^\w\s]/.test(password) && password === confirm) ? colors.primary : '#9CA3AF' }]} onPress={handleFinish} disabled={!(password.length >= 8 && /\d/.test(password) && /[^\w\s]/.test(password) && password === confirm)}>
