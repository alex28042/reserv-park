...
export default function ExploreScreen() {
  ...
  const handleOfferSubmit = (payload: OfferSpotPayload) => {
    ...
    const lat = payload.latitude ?? location?.latitude ?? currentRegion?.latitude ?? 40.4168;
    const lon = payload.longitude ?? location?.longitude ?? currentRegion?.longitude ?? -3.7038;
    const now = Date.now();
            ...
            <View style={styles.mapContainer}>
              {loading ? (
                ...
                <MapView
                  ...
                  showsScale={false}
                  mapType="standard"
                  customMapStyle={colorScheme === 'dark' ? require('@/constants/MapStyles').getDarkMapStyle() : undefined}
                >
...
// removed local style function; using constants/MapStyles
